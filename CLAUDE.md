...
- prompts/payments/story.yaml — selection criteria, news recognition, output format; instructs Claude to produce a concise briefing title. `output_format` explicitly tells the model to respond with the YAML only — no candidate-by-candidate deliberation or feed-quality commentary beforehand, even on a thin day (2026-08-04, see generator/client.py note above for the incident this addresses)
